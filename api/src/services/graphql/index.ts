			...
			auth_login: {
				...
				resolve: async (_, args, { req, res }) => {
					...
					if (env['ACCESS_TOKEN_COOKIE_NAME']) {
						res?.cookie(env['ACCESS_TOKEN_COOKIE_NAME'] as string, result['accessToken'], COOKIE_OPTIONS.accessToken);
					}

					if (args['mode'] === 'cookie') {
						res?.cookie(env['REFRESH_TOKEN_COOKIE_NAME'] as string, result['refreshToken'], COOKIE_OPTIONS.refreshToken);
					}
